...
　この記事では、まず、遺伝的アルゴリズムの仕組みや特徴、様々な分野への応用例について説明します。
...
株式の自動売買において、テクニカル指標のパラメータを遺伝子としてとらえて、遺伝的アルゴリズムを適用するという研究が存在するようです。
...
このようにパラメータが大量に存在したり、目的となる関数が複数個ある問題に対しても遺伝的アルゴリズムは有用なようです。
...
このため、次のような数学的な手法や従来の探索方法では解を探すのが困難な場合であっても、遺伝的アルゴリズムであれば適用できる場合があります。
...
遺伝的アルゴリズムによって求められた解は、常に最適な解であるとは限りません。
...
最後に、遺伝的アルゴリズムの考え方から、私たちが人生を生きる上でどのような教訓を得ることができるのか考えます。
